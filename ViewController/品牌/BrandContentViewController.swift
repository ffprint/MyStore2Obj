...
class BrandContentViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        webView.frame.size.height = SCREEN_H - 64
...
//        WebView.loadRequest(request)
        if self.title == "腕表详情"  {
            webView.frame.size.height = SCREEN_H - 64-46
            self.initViews()
        }
    }
    ...
    //腕表详情, 最下边 的收藏分享栏
    func initViews() -> Void {
        shareView.frame = CGRectMake(0, SCREEN_H-110, SCREEN_W, 49)
        ...
        for i in 0...3 {
            ...
            btn.setImage(UIImage.init(named: btnImg[i]), forState: UIControlState.Normal)
